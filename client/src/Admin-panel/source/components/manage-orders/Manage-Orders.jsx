		...
		td = data.map((order) => {
			return (
				<tr key={order._id}>
						...
						<td>
							{
								<ul>
									<li>
										<strong>Address:</strong> {order.Shipping_Info.address}
									</li>
									<li>
										<strong>City:</strong> {order.Shipping_Info.city}
									</li>
									<li>
										<strong>Street:</strong> {order.Shipping_Info.street}
									</li>
									<li>
										<strong>State:</strong> {order.Shipping_Info.state}
									</li>
									<li>
										<strong>Country:</strong> {order.Shipping_Info.country}
									</li>
								</ul>
							}
						</td>
					...
					<td>
						<strong>{order.totalPrice} PKR</strong>
					</td>
					...
					<td>
						{order.status === "shipped" ? (
							<button
								onClick={() => deliveredBtnHandler(order._id)}
								className={classes.deleteBtn1}
							>
								Delivered
							</button>
						) : (
							<button
								onClick={() => shipBtnHandler(order._id, order.products)}
								className={classes.updateBtn1}
							>
								Ship
							</button>
						)}
